# Preparing files for Wombat
# SNP Quality Control and Computation of Genomic Matrix

library(tidyr)
library(dplyr)

# How my SNP data should looks? The SNPfile should have animalID and SNP markers (one per column!)
...
write.table(tSNP_impdata, file="~/SNPCounts.dat", row.names = F, col.names = F, quote = F)
write.table(chrmap, file="~/chrmap", row.names = F, col.names = F, quote = F)
...
# Save inverse G matrix (in col format)
write.table(upperGinv, file="~/animal.gin", row.names = F, col.names = F, quote = F)
