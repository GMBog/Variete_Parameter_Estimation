...
INPUTDIR=/blue/mateescu/martinezbogg.wisc/microbiome_UFUofG/model_omics/GWAS/polygenicEff/
OUTPUTDIR=/blue/mateescu/martinezbogg.wisc/microbiome_UFUofG/model_omics/GWAS/polygenicEff/results
...
cp $INPUTDIR/phenoWO.dat pheno.dat
...
if [ $phenotype -eq 5 ]; then
        ...
        cp $INPUTDIR/parameterfile5.par parameterfile.par

        wombat --snap parameterfile.par > wombat.out

else

        cp $INPUTDIR/parameterfile67.par parameterfile.par
        sed -i "22 s/\([^ ]\+\)/$GV/" parameterfile.par
        sed -i "25 s/\([^ ]\+\)/$RV/" parameterfile.par
